...
positions <-
  list.files(path = meth_path, pattern = "*window_positions*csv") %>% 
  map_df(~read_csv(.)) %>%
df
position_files <- fs::dir_ls(path = meth_path, glob = "*window_positions*csv")
positions <- read_csv(position_files) %>%
  separate(chrm.window, sep='\\.', into = c("chromosome","window")) %>%
...
```{r}
windows <-
  list.files(path = meth_path, pattern = "*window_mean.*ia13.txt", full.names = TRUE) %>% 
  map_df(~read_csv(.x, id = 'file_name', col_names = FALSE)) 
windows <- lazy_dt(windows)
windows <- windows %>%
  separate(file_name, sep = '_', into = c("treatment","chromosome","ID"), fill="left") %>%
  mutate(ID = str_extract(ID, "[0-9]+")) %>%
  mutate(chromosome = str_extract(chromosome, "[0-9]+")) %>%
  mutate(treatment = case_when(
    treatment == "HE3" ~ "recovery",
    treatment == "HE2" ~ "hot",
    treatment == "HE1" ~ "pre",
    )) %>%
  as_tibble()
...
```{r}
biochems <- biochem %>%
  select(!c("ID","days","treatment","room","cohort")) %>%
  names()

j_plots = map(biochems, ~jitter_plots("treatment", .x) )
j_plots
```
